...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    const arya = {"class": "Rogue", "name": "Arya Stark", "phrases": ["Not today", "A girl has no name."]};
    expect(getCharacter('Arya')).toEqual(arya);

    // 3. Teste se a função retorna o objeto correto para o parâmetro 'Brienne',
    const brienne = {"class": "Knight", "name": "Brienne Tarth", "phrases": ["Im No Lady, Your Grace.", "I, Brienne Of Tarth, Sentence You To Die."]};
    ...
    expect(getCharacter('SKATE')).toBeUndefined();
  });
});
